...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var state = {
      ...
      activeTab: null,
      _pendingShowPath: null, // Monaco 未就绪时等待显示的文件路径，内容必达
     ...
     * 已是绝对路径即盘符或UNC时原样返回——消息流尾巴传入的路径可能是绝对路径，
     * 不能再用 root 拼接。 */
    ...
    var CSS = [
      ...
      /* 侧栏收起时切换单图标模式，frame 根节点的 data-sidebar-collapsed 属性：
       * 隐藏标题与描述，按钮收成 36x36 图标方块，避免溢出窄 rail；
       * 尺寸与边缘样式对齐 DSH 原生「新建会话」图标按钮，36x36、radius 12px、透明边框，
       * 收起态 specificity 高于 .dsh-cc-trigger:hover，故 hover 时边框保持透明、仅背景变化 */
    ...
    function EditorPanel(props) {
      ...
      // editorInstanceRef 会残留指向已分离容器的旧实例，再次打开时因实例
      // 非空而跳过重建，导致「标签在、内容无」。故在无标签时主动销毁实例，
      // 让下次打开走 createEditor 重建 + 冲刷 pending，保证内容必达。
      ...
      function createEditor(container) {
        ...
        if (state._pendingShowPath) {
          var pending = state._pendingShowPath
     ...
     * 不能直接读 textContent——尾巴 span 注入后 textContent 会带上「古法编程」，
     * 重扫时若据此覆盖 dataset.path 会污染路径。
     */
    function btnPath(btn) {
      var cached = btn.getAttribute && btn.getAttribute('data-cc-path')
    ...
    var plugin = {
      ...
      // slots 与 sessions 都由客户端核心插件提供，声明进 inject 让 cordis
      // 等服务就绪后再 apply，避免 apply 时 ctx.get('sessions') 尚未就绪。
