 ...
 * 通过 ctx.connection.rpc.handle('/classic-coding', ...) 注册独立 RPC 通道，
...
/** 依赖的服务：fs 即文件系统；connection 在 apply 内显式等待 */
const inject = ['fs']
 ...
 * intercept 会抛出 'already has an interceptor'，因此插件端点必须走 handle 独立通道。
 *
 ...
 * 处理 writeFile：写入文件内容。
 *
 * 注意：这里刻意不走 `ctx.fs`，那是被 DSH 文件沙箱包过的、会按 agent 权限裁决的实现，
 * 而是直接用 Node 原生 `fs.writeFile` 写盘。
 * 这个插件本质是本地编辑器，`Ctrl+S` 保存与 agent 完全无关，不应被 DSH 的
 * agent 沙箱/权限链拦截，否则会报 `file access denied under workspace-write mode`。
 * 因此保存操作与 agent 的访问模式、会话 override 都无关，就是普通本地磁盘写入。
 *
 * @param {import('@deepseek-ai/cordis').Context} ctx
